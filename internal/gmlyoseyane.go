...
func ExposYose(f *os.File, id, fid string, vcnt int, list [][]float64, elv, btm, toph, hisashi, keraba, incline, yaneatu float64, story int) {
	log.Println("寄棟屋根")
	// 屋根モデル（寄棟屋根）
	// 4つの頂点座標の定義
	x1 := list[0][0]
	...
	mbt2 := (mbt01 + mbt02) / 2
	// 寄棟屋根の棟端の上端高さ
	myp01 := mbt1 + yaneatu*math.Sqrt(1+math.Pow(incline, 2))
	myp02 := mbt2 + yaneatu*math.Sqrt(1+math.Pow(incline, 2))
	myp1 := (myp01 + myp02) / 2
	myp2 := (myp01 + myp02) / 2
